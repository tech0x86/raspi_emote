...
def generate_voice(text, force_generate=False):
    ...
    Args:
        text (str): 生成するテキスト
        force_generate (bool): ニュース以外でも強制的に生成するかどうか
    """
    global core
    
    # ニュース以外ではVOICEBOXを使わない設定の場合
    if USE_VOICEBOX_ONLY_FOR_NEWS and not force_generate:
        # ニュースか判定（簡易的な判定）
        is_news = "ニュース" in text or "最近の" in text or "今日の出来事" in text
        if not is_news:
            add_log(f"音声合成スキップ: {text[:20]}...")
            return None
    ...
    try:
        ...
        # ここで本来はVOICEVOX Coreでの音声生成を行う
        # 実際の処理は省略（時間がかかるため）
        
        # ダミーのファイル作成（実際の実装では、VOICEVOXで生成したファイルが保存される）
        # この部分は実際の環境では適切に実装する必要があります
...
def fetch_news():
    ...
    try:
        ...
        if result["status"] == "ok" and result["articles"]:
            ...
            add_log(f"ニュース更新: {len(news_data)}件取得")
            return True
...
def update_gui():
    ...
    if time_label:
        time_label.config(text=f"現在時刻: {current_datetime}")
...
def create_gui():
    ...
    # 現在時刻ラベル
    time_label = tk.Label(top_frame, text="現在時刻: -", font=("Helvetica", 14))
        ...
        while True:
            ...
            if current_distance < 150:
                ...
                if current_time - last_interaction_time > 30:
                    ...
                    # 音声生成と再生（音声なしに設定）
                    audio_file = generate_voice(message, force_generate=False)
                    if audio_file:
                ...
                # 1m以内に近づいた場合はニュース提供
                    ...
                    add_log(f"ニュース: {topic}")
                    
                    # ニュース応答はVOICEBOXで生成（force_generate=True）
                    audio_file = generate_voice(topic, force_generate=True)
                    if audio_file:
                        audio_queue.put(audio_file)
                ...
                # 0.5m以内に近づいた場合はより対話的な会話
                    ...
                    # 音声生成と再生（音声なしに設定）
                    audio_file = generate_voice(response, force_generate=False)
                    if audio_file:
                        audio_queue.put(audio_file)
            ...
            # 人がいない場合のアイドル行動
                ...
                if idle_counter >= 6000:
                    ...
                    # 音声生成と再生（音声なしに設定）
                    audio_file = generate_voice(idle_topic, force_generate=False)
                    if audio_file:
                        audio_queue.put(audio_file)
